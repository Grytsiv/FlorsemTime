...
const CompanyCard: React.FC<ICompanyCard> = ({item, onPress}) => {
  ...
  if ('daysRemained' in data && data.type === 1) {
    companyValidToDate = new Date(
            moment(now)
              .add(data.daysRemained + 1, 'days')
              .toDate(),
          ).valueOf();
    isDateValid = data.daysRemained >= 0;
  } else if ('expirationDate' in data && data.type === 2) {
    companyValidToDate = new Date(moment(data.expirationDate).toDate()).valueOf();
    isDateValid = moment(data.expirationDate).valueOf() >= now;
  } else if ('agBalance' in data && 'smsBalance' in data && data.type === 0) {
    companyValidToDate = new Date(moment().endOf('month').toDate()).valueOf();
    isDateValid = data.agBalance >= 0 && data.smsBalance >= 0;
  }
